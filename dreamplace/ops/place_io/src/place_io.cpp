                ...
                if (row.orient() == Orient::vflip(node.orient()))  // only vertically flipped
                {
                    node.setOrient(row.orient());
                           ...
                           Orient::hflip(Orient::vflip(
                               node.orient())))  // both vertically and horizontally flipped
                {
                    // flip vertically
...
PlaceDB place_io_forward(pybind11::list const& args) {
    // char buf[256];
    // DREAMPLACE_NAMESPACE::dreamplaceSPrint(DREAMPLACE_NAMESPACE::kINFO, buf, "reading input files
    // takes %%t seconds CPU, %%w seconds real\n"); boost::timer::auto_cpu_timer timer (buf);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        "write",
        [](DREAMPLACE_NAMESPACE::PlaceDB const& db, std::string const& filename,
           DREAMPLACE_NAMESPACE::SolutionFileFormat ff,
           pybind11::array_t<float, pybind11::array::c_style | pybind11::array::forcecast> const& x,
           pybind11::array_t<float, pybind11::array::c_style | pybind11::array::forcecast> const&
               y) { return write(db, filename, ff, x, y); },
        "Write Placement Solution (float)");
    m.def(
        "write",
        [](DREAMPLACE_NAMESPACE::PlaceDB const& db, std::string const& filename,
           DREAMPLACE_NAMESPACE::SolutionFileFormat ff,
           pybind11::array_t<double, pybind11::array::c_style | pybind11::array::forcecast> const&
               x,
           pybind11::array_t<double, pybind11::array::c_style | pybind11::array::forcecast> const&
               y) { return write(db, filename, ff, x, y); },
    ...
    m.def(
        "pydb",
        [](DREAMPLACE_NAMESPACE::PlaceDB const& db) { return DREAMPLACE_NAMESPACE::PyPlaceDB(db); },
