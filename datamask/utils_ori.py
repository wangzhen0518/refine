...
def random_guiding(node_id_ls, placedb, grid_num, grid_size):  # 将所有macro随机放置
    ...
    for node_id in node_id_ls:
        ...
        scaled_width = math.ceil(x / grid_size)
        scaled_height = math.ceil(y / grid_size)
        placedb.node_info[node_id]["scaled_width"] = scaled_width
        placedb.node_info[node_id]["scaled_height"] = scaled_height
...
def greedy_placer_with_init_coordinate(node_id_ls, placedb, grid_num, grid_size, place_record):
    ...
    for node_id in node_id_ls:
        ...
        position_mask[: grid_num - scaled_width, : grid_num - scaled_height] = 1  #! 这么做的原因？为什么是 grid_num - scaled_width, 而不是 scaled_width
        wire_mask = np.ones((grid_num, grid_num)) * 0.1

        for key1 in placed_macros.keys():
            bottom_left_x = max(0, placed_macros[key1]["loc_x"] - scaled_width + 1)
            bottom_left_y = max(0, placed_macros[key1]["loc_y"] - scaled_height + 1)
            top_right_x = min(grid_num - 1, placed_macros[key1]["loc_x"] + placed_macros[key1]["scaled_width"])
            top_right_y = min(grid_num - 1, placed_macros[key1]["loc_y"] + placed_macros[key1]["scaled_height"])
        ...
        placed_macros[node_id] = {
            ...
            "center_loc_x": center_loc_x,
            "center_loc_y": center_loc_y,
            "bottom_left_x": chosen_loc_x * grid_size + 452,
...
def greedy_local_search(queue, placedb, placed_macros, grid_size, grid_num):
    ...
    for key in queue:
        ...
        scaled_width = math.ceil(x / grid_size)
        scaled_height = math.ceil(y / grid_size)
        position_mask[: grid_num - scaled_width, : grid_num - scaled_height] = 1

        for key1 in placed_macros.keys():
            bottom_left_x = max(0, int(placed_macros[key1]["loc_x"] - placed_macros[key1]["scaled_width"] + 1))
            bottom_left_y = max(0, int(placed_macros[key1]["loc_y"] - placed_macros[key1]["scaled_height"] + 1))
            top_right_x = min(grid_num - 1, int(placed_macros[key1]["loc_x"] + placed_macros[key1]["scaled_width"]))
            top_right_y = min(grid_num - 1, int(placed_macros[key1]["loc_y"] + placed_macros[key1]["scaled_height"]))
        ...
        if best_hpwl < baseline_hpwl:
            ...
            placed_macros[key] = {
                ...
                "x": x,
                "y": y,
                "center_loc_x": chosen_center_loc_x,
                "center_loc_y": chosen_center_loc_y,
                "bottom_left_x": chosen_loc_x * grid_size,
                "bottom_left_y": chosen_loc_y * grid_size,
            }
...
def bo_placer(node_id_ls, placedb, grid_num, grid_size, place_record, csv_writer, csv_file):
    ...
    print("hpwl:", hpwl)
    csv_writer.writerow([hpwl, time.time()])
    csv_file.flush()
