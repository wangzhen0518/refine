...
class MacroPort : public Object {
  public:
    ...
    MacroPort& setBbox(box_type const& b) {
        m_bbox = b;
        return *this;
    }
...
inline MacroPort::MacroPort() : MacroPort::base_type(), m_bbox(), m_vBox(), m_vLayer() {}
inline MacroPort::MacroPort(MacroPort const& rhs) : MacroPort::base_type(rhs) {
    copy(rhs);
}
...
/// since a pin is usually a rectilinear polygon, the decomposed rectangles are introduced to store
/// its shape
...

{
  ...
  public:
    ...
    MacroPin& setName(std::string const& s) {
        m_name = s;
        return *this;
    }

    SignalDirect const& direct() const { return m_direct; }
    MacroPin& setDirect(SignalDirect const& d) {
        m_direct = d;
        return *this;
    }

    box_type const& bbox() const { return m_bbox; }
    MacroPin& setBbox(box_type const& b) {
        m_bbox = b;
        return *this;
    }
...
inline MacroPin::MacroPin()
    : MacroPin::base_type(), m_name(""), m_direct(), m_bbox(), m_vMacroPort() {}
inline MacroPin::MacroPin(MacroPin const& rhs) : MacroPin::base_type(rhs) {
    copy(rhs);
}
...
inline void deriveMacroPortBbox(MacroPort& mp) {
    ...
    box_type box(
        std::numeric_limits<coordinate_type>::max(), std::numeric_limits<coordinate_type>::max(),
        std::numeric_limits<coordinate_type>::min(), std::numeric_limits<coordinate_type>::min());
...
inline void deriveMacroPinBbox(MacroPin& mp) {
    ...
    for (std::vector<MacroPort>::const_iterator it = vMacroPort.begin(), ite = vMacroPort.end();
         it != ite; ++it)
